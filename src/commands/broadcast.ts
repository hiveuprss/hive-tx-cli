...
function getAccountName(config: any, options: any): string {
...
const commentCmd = new Command('comment')
  ...
  .action(async (options) => {
    ...
    const jsonMetadata = options.tags
      ? JSON.stringify({ tags: options.tags.split(',').map((t: string) => t.trim()) })
      : '';
...
const customJsonCmd = new Command('custom-json')
  ...
  .action(async (options) => {
    ...
    const requiredPostingAuths = options.requiredPosting
      ? options.requiredPosting.split(',').map((a: string) => a.trim())
      : [account];

    const requiredAuths = options.requiredActive
      ? options.requiredActive.split(',').map((a: string) => a.trim())
      : [];
